...
RUN git clone https://github.com/josephjw/apprtc.git
...
RUN echo -e "#!/bin/sh\n" > /go/start.sh \
    && echo -e "`pwd`/google-cloud-sdk/bin/dev_appserver.py --ssl_certificate_path `pwd`/apprtc/ca.cert.pem --ssl_certificate_key_path `pwd`/apprtc/ca.key --host 0.0.0.0 --enable_host_checking=false `pwd`/apprtc/out/app.yaml &\n" >> /go/start.sh
